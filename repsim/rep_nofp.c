...
int effstrats[NUMSG][NUMAO] = { {1,1},{1,1},{1,1},{1,0} };
...
int binsize = 500;
int numbins = 10000/binsize;		/*number of histogram bins*/
...
/*for (i=0;i<MAXDSTRATS;i++)	{
	if (defstrats[i].SG[activeSG].active == 1 && defstrats[i].SG[1].active == 0
		&& defstrats[i].SG[2].active == 0 && defstrats[i].SG[3].active == 0 &&
		defstrats[i].SG[4].active == 1)
...
/*for (i=1;i<MAXDSTRATS;i++)	{
	printf("astrat:%d dstrat: %d DP:%.4f\n",astrat,i,calcPayoff(defstrats[i],attstrats[astrat]));
}*/
/*for (i=1029;i<1030;i++)	{
	printf("cost-%d:%.2f\n",i,calcCost(defstrats[i],attstrats[0]));
	printDStrat(fp,defstrats[i]);
}*/
...
struct sginfo dstratinfo[NUMSG] = {dualcsi,divi,smmsi,rdai};
...
if (dstrat.SG[3].active == 1)	{
	...
	numinsp = floor((SIMYEAR)/dstrat.SG[3].freq);
