...
/*find max DP and defender payoff that yields it for given attacker strat*/
...
#define BUDGET		4000		/*defender budget*/
...
/*OTHER DP'S*/
float bgDP = 0;				/*background DP-- applied every DAY-- for dailyDP[k][t]
...
for (i=0;i<MAXDSTRATS;i++)	{
	/*printf("checking strat %d\n",i);*/
	if (calcCost(defstrats[i],attstrats[astrat])<=BUDGET)
		if (calcPayoff(defstrats[i],attstrats[astrat]) > maxDP)	{
			beststrat = i;
			maxDP = calcPayoff(defstrats[i],attstrats[astrat]);
		}
}
...
printDStrat(defstrats[beststrat]);

printf("best defender strategy: %d\n",beststrat);
printf("payoff: %.6f\n",maxDP);
...
/*the bg DP occurs every day-- store as k+1 SG*/
...
printf("cumDP: %.12f\n",cumDP);
printf("mattype: %.2f\n",mattype);	*/
float payoff = cumDP/pow((mattype * totalU),ALPHA);
