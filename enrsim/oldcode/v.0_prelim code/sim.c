...
#define MBCOST 		25		/*annual equipment cost for mbalance-- includes maintenance*/
#define VLCOST		48		/*annual equipment cost for videolog*/
#define NDACOST	  	41		/*annual equipment cost for nda*/
#define	DACOST		5		/*annual equipment cost for da*/
#define	VTCOST		80		/*annual equipment cost for videotrans*/
#define	CEMOCOST	33		/*annual equipment cost for CEMO*/
...
#define	ASSESSCOST	0.8		/*assessment time- per day cost*/
...
#define PSEALSDP		0.85	/*conditional DP for a single passive seal-- DP given
								that a sealed cylinder is attacked*/
#define ASEALSDP	0.40	/*DP for a single active seal*/

#define NUMCYLFALSE	1.0		/*number of product cylinders falsified-- all over-enriched
...
/*OTHER DP'S*/
float bgDP = 0.001;				/*background DP-- applied every DAY-- for dailyDP[k][t]
...
float activesealcost = 1;		/*$100 per seal*/
float swipecost = 6.25;			/*$625 per DA swipe*/
...
float xp[3] = {0.050,0.50,0.90};			/*product enrichment*/
...
for (j = 0;j<numastrats;j++)
	astrats[j] = j+4;

/*inspection*/
/*int dstrats[8] = {45,1260,2475,3690,4905,10530,16155,21780};*/
/*videotrans*/
/*int dstrats[2] = {15,30};*/
/*pseals*/
/*int dstrats[4] = {450,855,7155,9405}; */
/*aseals*/
/*int dstrats[2] = {5,10};*/
/*nda*/
/*int dstrats[4] = {180,315,5355,5805};*/
/*da*/
/*int dstrats[4] = {90,135,4995,5040};*/

/*cemo*/
/*int dstrats[4] = {1,2,3,4};*/
...
/*for (j=0;j<numastrats;j++)	{
	printf("dstrat:%d,astrat:%d\n",9405,astrats[j]);
	calcDP(defstrats[9405],attstrats[astrats[j]]);
	printf("\n\n");
}*/

calcDP(defstrats[dstrat],attstrats[astrat]);
/*calcDP(defstrats[1],attstrats[75]);*/



/*long maxcost = 0;
int expensivedstrat = 0;
int expensiveastrat = 0;
int cheapestdstrat = 0;
int cheapestastrat = 0;
long mincost = 100000;

/*for (j=0;j<MAXASTRATS;j++)
	for (i=0;i<MAXDSTRATS;i++)
		if (calcCost(defstrats[i],attstrats[j]) > maxcost)	{
			maxcost = calcCost(defstrats[i],attstrats[j]);
			expensivedstrat = i;
			expensiveastrat = j;
		}
...
for (j=0;j<MAXASTRATS;j++)
	for (i=0;i<MAXDSTRATS;i++)
		if (calcCost(defstrats[i],attstrats[j]) < mincost && 
			calcCost(defstrats[i],attstrats[j]) != 0)	{
			mincost = calcCost(defstrats[i],attstrats[j]);
			cheapestdstrat = i;
			cheapestastrat = j;
		}
printf("%d/%d is cheapest strategy: %ld\n",cheapestdstrat,cheapestastrat,mincost);
printf("%d/%d is priciest strategy: %ld\n",expensivedstrat,expensiveastrat,maxcost); */

/*printf("cost of %d/%d: %ld\n",dstrat,astrat,calcCost(defstrats[dstrat],attstrats[astrat])); */
...
for (k=0;k<NUMSG+1;k++){
	...
	printf("cumDP for safeguard %d: %.3f\n",k,cumSGDP[k]);
}
...
for (k=0;k<NUMSG+1;k++)	{
	for (t=0;t<m;t++)
		cumSGDP[k] = 1-(1-cumSGDP[k])*(1-dailyDP[k][t]);
	printf("cumulative DP for safeguard %s:%.3f\n",dstrat.SG[k].name,cumSGDP[k]);
}
printf("cumulative DP for background dp:%.3f\n",cumSGDP[NUMSG]);*/
...
/*attack duration*/
int duration = 0;
/*number of inspections for strategy pair*/
int numinsp = 0;

if (astrat.AO.tend == EMPTY)
	duration = 0;
else
	duration = astrat.AO.tend;	


/*cost for inspection*/
if (dstrat.SG[0].active == 1)	{
	/*number of inspections*/
	numinsp = floor((duration + EXTRADAYS)/dstrat.SG[0].freq);
	/*time costs*/
...
if (dstrat.SG[1].active == 1)	{
	/*number of inspections*/
	numinsp = floor((duration + EXTRADAYS)/dstrat.SG[1].freq);
	/*time costs*/
	pseals = INSPCOST*numinsp;
	if (dstrat.SG[1].freq == freq[0])
		pseals = pseals * freq[1]/freq[0];
		
	/*add in per seal costs-- assume you have to pay to seal for attack period*/ 

	pseals = pseals + sealcost * floor(duration/(dstrat.SG[1].freq+psealsi.analysis)) *
				dstrat.SG[1].number * (numcyl[0]+numcyl[1]);
				
}
/*nda cost*/
if (dstrat.SG[2].active == 1)	{
	/*number of inspections*/
	numinsp = floor((duration + EXTRADAYS)/dstrat.SG[2].freq);
	/*time costs*/
	nda = INSPCOST*numinsp;
...
if (dstrat.SG[3].active == 1)	{
	/*number of inspections*/
	numinsp = floor((duration + EXTRADAYS)/dstrat.SG[3].freq);
	/*time cost*/
	da = INSPCOST*numinsp;
		
	/*per swipe cost*/
	da = da + swipecost * floor(duration/(dstrat.SG[3].freq+dai.analysis))*dstrat.SG[3].number *
				numcyl[1];
...
if (dstrat.SG[4].active == 1)	{
	/*number of days of assessment*/
	numinsp = duration+EXTRADAYS;
	/*time cost*/
...
if (dstrat.SG[5].active == 1)	{
	...
	aseals = activesealcost * dstrat.SG[5].number * NUMCASC;
