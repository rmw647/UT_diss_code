...
#define IT			4		/*number of iterations for FP algorithm*/
...
/*make vup biggest number in columns*/
...
for (j=0;j<MAXASTRATS;j++)
	if (payoff[currentrow][j] == -1)
		payoff[currentrow][j] = calcDP(defstrats[currentrow],attstrats[currentcol]);
...
/*float DP = calcDP(defstrats[24],attstrats[1]); 
