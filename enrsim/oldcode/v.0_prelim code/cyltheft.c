/*use to find certain defender strategies*/
...
for (i=0;i<MAXDSTRATS;i++)
	if (defstrats[i].SG[0].active == 1 && defstrats[i].SG[1].active == 0 &&
		defstrats[i].SG[2].active == 0 && defstrats[i].SG[3].active == 0 &&
		defstrats[i].SG[4].active == 0 && defstrats[i].SG[5].active == 0 &&
		defstrats[i].SG[6].active == 0)	
		printDStrat(defstrats[i]);

for (i=0;i<MAXDSTRATS;i++)
	if (defstrats[i].SG[specSG].active == 1)
		for (k =0;k<specSG;k++)
			if (defstrats[i].SG[k].active == 0)
				for (k=specSG+1;k<NUMSG;k++)
					if (defstrats[i].SG[k].active == 0)
						printf("strategy %d\n",i);
