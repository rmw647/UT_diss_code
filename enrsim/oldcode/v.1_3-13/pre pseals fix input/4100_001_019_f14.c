...
#define BUDGET		4100		/*defender budget*/
...
#define ALPHA		0.19		/*exponent in denominator of payoff calculation--
							payoff = DP/(FOM*mass)^alpha*/
#define SIMYEAR	360	/*360 days in a simulation year*/

char fileName[] = "results_4100_001_019_f14.txt";
...
int sifreq[2] = {14,28};		/*special inspection frequency options*/
				...
				if (effstrats[k][actastrat] == 1){	/*if safeguards is effective against att. strat*/
					...
					if (strcmp(astrat.AO.name,"matcyl") == 0) {	
						...
						if (strcmp(dstrat.SG[k].name,"pseals")==0)		{
							...
							if ((attackday > t-dstrat.SG[k].freq) &&
									(attackday <= t))
									attackhappened = 1;
