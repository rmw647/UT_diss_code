/*enrichment simulation for NEW payoff and NO FP*/
...
#define VIDEOLOGDP	0.43f	/*initial DP for videolog*/
...
long plotdata[59];		/*cumulative distribution fcn*/
long pdfdata[59];		/*probability distribution fcn data*/
...
int dstrat = 1846;
int astrat = 300;
int astrats[20] = {0,5,24,46,101,155,156,169,185,198,300,304,319,276,171,
                 311,308,305,319,320};
                 
start = clock();		/*used to calculate time for sim to run*/
...
/*fp = fopen("defenderstrats.txt", "w");
  if (fp == NULL) {
	 printf("I couldn't open results.dat for writing.\n");
	 exit(0);
  }
for (i=0;i<MAXDSTRATS;i++)	
	fprintf(fp,"strategy:%d SG0:%d SG4:%d SG5:%d SG6:%d SG7:%d\n",
			defstrats[i].uniqueID,defstrats[i].SG[0].active,defstrats[i].SG[4].active,
			defstrats[i].SG[5].active,defstrats[i].SG[6].active,defstrats[i].SG[7].active); 
...
/*int count = 0;
for (i=0;i<MAXDSTRATS;i++)
	if (calcCost(defstrats[i],attstrats[276])<=1550)
		count++;
...
for (i=2;i<MAXDSTRATS;i++)
	if (calcCost(defstrats[i],attstrats[astrat])<calcCost(defstrats[cheapeststrat],attstrats[astrat]))
		cheapeststrat = i;

for (i=0;i<MAXDSTRATS;i++)
	if (calcCost(defstrats[i],attstrats[astrat])>calcCost(defstrats[mostexpensivestrat],attstrats[astrat]))
		mostexpensivestrat = i;
		
printf("cheapest strategy:%d, cost:%ld\n",cheapeststrat,calcCost(defstrats[cheapeststrat],
											attstrats[astrat]));
printf("most expensive strategy:%d, cost:%ld\n",mostexpensivestrat,
									calcCost(defstrats[mostexpensivestrat],attstrats[astrat]));
...
for (i=0;i<MAXDSTRATS;i++)
	for (j=1;j<60;j++)
		if (calcCost(defstrats[i],attstrats[276])<j*100)	{
			plotdata[j-1]++;
			break;
		}
...
/*long count = 0;
for (i=0;i<MAXDSTRATS;i++)	{
	if (defstrats[i].SG[0].active == 1)
		count++;
	else if (defstrats[i].SG[4].active == 1)
		count++;
	else if (defstrats[i].SG[5].active == 1)
		count++;
	else if (defstrats[i].SG[6].active == 1)
		count++;
	else if (defstrats[i].SG[7].active == 1)
		count++;
}
...
printDStrat(defstrats[204120]);
printDStrat(defstrats[204210]);*/
...
float DP;
int someSGactive = 0;			/*identifies 0 defender strategy*/

/*for (k=0;k<NUMSG;k++)
	if (dstrat.SG[k].active != 0)	{
		someSGactive = 1;
		break;
	}
